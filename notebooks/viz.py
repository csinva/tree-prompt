...
def plot_perf_curves_individual(
    rp,
    x="max_depth",
    ...
    fig, axes = plt.subplots(
        figsize=(C * 2.5, R * 2.5), nrows=R, ncols=C, layout="constrained"
    )
    for i in range(R * C):
        # plt.subplot(R, C, i + 1)
        if R * C > 1:
        ...
        for k, g in rd.groupby(by=groupings, sort=False):
            ...
            if special_hlines and k == "manual_gbdt" or k == "manual_rf":
                # horizontal line
...
if __name__ == "__main__":
    ...
    dt, strs_array = imodelsx.viz.extract_sklearn_tree_from_llm_tree(
        model, n_classes=2, dtreeviz_dummies=False
    )
    sklearn.tree.plot_tree(
        dt,
        feature_names=strs_array,
        class_names=["neg", "pos"],
        precision=2,
        rounded=True,
    )
    ...
    dt, strs_array = imodelsx.viz.extract_sklearn_tree_from_llm_tree(
        model, n_classes=2, dtreeviz_dummies=True
    )
